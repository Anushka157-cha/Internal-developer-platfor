...
export default function ServiceDetailPage() {
  ...
  const healthCheckMutation = useMutation({
    mutationFn: async () => {
      return api.post(`/services/${id}/health-check`)
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      alert(err.response?.data?.message || 'Health check probe failed')
    },
  })
        ...
        <div className="flex items-start justify-between">
            ...
            <div className="flex items-center mt-2 space-x-4">
              ...
              {(service as any)?.responseLatencyMs !== undefined && (
                <span className="text-xs text-gray-500 font-mono">
                  Latency: {(service as any)?.responseLatencyMs}ms
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="flex items-center px-3 py-2 bg-slate-100 text-slate-700 border border-slate-300 rounded-lg hover:bg-slate-200 transition-colors text-sm font-semibold disabled:opacity-50"
              title="Probe registered HTTP health endpoint"
            >
              <RefreshCw className={`h-4 w-4 mr-1.5 ${healthCheckMutation.isPending ? 'animate-spin' : ''}`} />
              {healthCheckMutation.isPending ? 'Probing...' : 'Run Health Check'}
            </button>
            ...
            {canDeploy && (
              <button
                ...
                className="flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 text-sm font-semibold"
              >
                <Rocket className="h-4 w-4 mr-2" />
                {deployMutation.isPending ? 'Queuing...' : 'Deploy'}
              </button>
            )}
          </div>
        </div>
